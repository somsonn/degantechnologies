...
function countTrashTeamMembers(id) {
  if (removedTeamMemberIds.value.includes(id)) {
    removedTeamMemberIds.value = removedTeamMemberIds.value.filter(
...
const hideTeamMember = () => {
  axios.defaults.headers.common["Authorization"] = token.value;
  axios
    .post("./api/team-members/hidden", { id: removedTeamMemberIds.value })
    .then((res) => {
          ...
          <table class="w-full text-sm text-left text-gray-500">
            ...
            <tbody v-for="member in paginatedData" :key="member.id">
              <tr class="bg-white border-b hover:bg-gray-50">
                ...
                <td>
                  <router-link
                    :to="{ name: 'editTeamMember', params: { id: member.id } }"
