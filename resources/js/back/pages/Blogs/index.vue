...
const searching = computed(() => {
    ...
    return allblogs.value.filter(item => item.title.toLowerCase().includes(keyword));
});
    ...
    <div class="bg-gray-100">
        ...
        <div class="m-4 rounded">
            <div class="w-full bg-white border-b-2 rounded border-gray-200 flex flex-row justify-between">
                <h1 class="font-bold text-gray-500 text-lg p-4 capitalize">Blogs table</h1>
                <router-link to="/post"
            ...
            <div class="flex bg-white flex-col items-center justify-center">
                ...
                <div class=" w-full  relative p-2 overflow-x-auto shadow-md sm:rounded-lg xs:p-5">
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                        ...
                        <tbody v-for="blogs in paginatedData" :key="blogs.id">
                            <tr v-if="blogs" class="bg-white border-b hover:bg-gray-50">
                                <td @click.prevent="countandtrash(blogs.id)" class="w-4 p-4">
                                    <div class="border-4 border-gray-200 text-center rounded w-5 h-5">
                                        <p v-if="dletedId.includes(blogs.id)"><i  class="material-icons text-2xl font-bold text-green-600">done</i></p>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <router-link :to="{ name: 'edit', params: { id: blogs.id } }"
